...
Thanks for using `zenodo-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

Open one from the **Issues** tab and pick the **Bug Report** or **Feature Request** form, or go directly to https://github.com/cyanheads/zenodo-mcp-server/issues/new?template=bug_report.yml. Both are structured, and filling in the fields is what makes an issue actionable. Anything that fits neither can be a plain issue — a half-formed idea in your own words is fine.

## Server bug or framework bug?

`zenodo-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:
