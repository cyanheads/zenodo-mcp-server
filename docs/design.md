...
- **HTML descriptions** (`metadata.description`, `additional_descriptions[].description`) are converted to plain text by a deterministic local `htmlToText()` (`src/services/zenodo/html-to-text.ts`), which runs in this order:
  ...
  This conversion is the one documented transformation of upstream text. `structuredContent` otherwise carries upstream strings verbatim. Every step is linear on hostile input: descriptions are converted for every search hit, and the earlier regex forms (`/<[^>]*>/g` on a run of `<`, the link pattern on unclosed `<a` tags, `/[ \t]+$/gm` on a long run of spaces) took 1–8 s on 100 KB. Tag patterns stop at the next `<` or `>`, tag stripping and link inlining are single forward scans, and trailing blanks are trimmed per line by index.
- **`format()` inline slots** (titles, names, keywords, file keys, ZIP member paths, version labels, community titles, tombstone removal reasons, echoed inputs, and every identifier-like upstream value: DOIs, ORCIDs, rights and license ids, identifier schemes and relations, access fields, MIME types, checksums, dates) pass through `inline(s)`, which replaces `/[\r\n\u0085\u2028\u2029]+/g` with a single space (NEL and the Unicode line/paragraph separators render as line breaks in some clients).
...
|:--|:--|:--|:--|
| `query` | optional string ≤1000 (blank → unset, trimmed) | `q` (balanced, then wrapped in parentheses, when filters are also composed; see [Query balancing](#query-balancing)) | Default operator is OR (`climate model` 591,127 vs `climate AND model` 25,766). Pre-checks (below) reject a whole-identifier query and an unpaired `/`; the `.describe()` names the first so a caller routes a bare DOI to `zenodo_get_record` up front |
...
<a id="query-balancing"></a>**Query balancing.** Zenodo matches a `q` it cannot parse as plain text instead of rejecting it, so a malformed caller query silently turns every composed filter clause into an optional term. Verified 2026-09-24 with `license: mit`: `(climate) AND metadata.rights.id:"mit"` → 2,015; `(climate "x) AND …` → 4,535,440; `(climate\) AND …` → 4,411,040; `(climate) OR (*) AND …` → 71,471. When filter clauses compose with the query, `balanceQuery()` (`query-builder.ts`) first drops leading and trailing boolean operators (`AND`, `OR`, `NOT`, `&&`, `||`, a lone `+`/`-`/`!`), completes a dangling `\` as `\\`, closes an unterminated quote or `/…/` regex, escapes a `)` that closes no group as `\)`, and closes any group left open. The balanced forms return the filtered totals (2,015, 3,354, 71,472). A well-formed query is unchanged, a query with no filters is sent as written, and `effectiveQuery` echoes exactly what was sent. Interior syntax errors are not repaired (see Known Limitations).
...
`download_url` for every file is `https://zenodo.org/api/records/{recid}/files/{key}/content`, with each `/`-separated key segment `encodeURIComponent`-encoded. Keys contain `/`, spaces, and parentheses (`scikit-learn/scikit-learn-1.9.1.zip`, `D6.1 (versione sottomessa).pdf`); a raw `/` and `%2F` both resolve. Encoding leaves a `.` or `..` segment literal, and the URL parser resolves it (`…/container/../../../../me` → `/api/me`), so every request path built from a key or member path refuses one (`filePath()` in `zenodo-service.ts`, a `ValidationError` before any request).
...
| `member_not_found` | `NotFound` | Upstream 404 on the member path, or a member path with a `.` or `..` segment (refused before any listing or member request: it would resolve to another zenodo.org endpoint) | Call zenodo_list_files with archive_key set to this key to see the member paths. |
...
- **Redirects stay on zenodo.org.** Every fetch uses `redirect: 'manual'`. A `follow` request (the default; the record GET follows a concept id's 302) follows a 301/302/303/307/308 itself, at most 3 hops, and only while the `Location` resolves to the `https://zenodo.org` origin; any other scheme, host, or port is refused as a non-retryable `ServiceUnavailable` without requesting it. The `Authorization` header is rebuilt per hop, so the token never leaves zenodo.org (Bun 1.4 and Node 26 also drop it on a cross-origin redirect, verified, but the boundary does not rely on that). A `manual` request (`versions/latest`) gets the 3xx itself.
- **Body cap.** JSON and citation bodies are streamed through `readCapped` and refused past 32 MiB (the heaviest upstream page observed is 8.4 MB) as a non-retryable `ServiceUnavailable`, never buffered whole.
...
- **A query Zenodo cannot parse drops the filters.** Balancing ([Query balancing](#query-balancing)) closes unbalanced delimiters and edge operators, but an interior syntax error still makes Zenodo match the whole `q` as plain text, so the filter clauses stop narrowing: `(climate AND AND model) AND metadata.rights.id:"mit"` → 4,442,737, and `metadata.title:` with no value, an unterminated range `[2020 TO`, `climate~~`, and an empty group `()` behave the same (verified 2026-09-24). Closing this needs a local parser for Zenodo's full query grammar; `community` (a separate parameter) and `all_versions` are unaffected.
- **The token authenticates as its owner.** Zenodo scopes a personal token's API actions, not whom reads run as, so a configured `ZENODO_ACCESS_TOKEN` is expected to make record reads and searches see what its account can see, restricted records it owns included (unverified: no token was used in probing). Every caller of a hosted server shares that identity. Create the token on a dedicated account that owns no restricted records and belongs to no restricted communities. `zenodo_read_file` and `zenodo_list_files` still refuse content and manifests whenever `access.files` is `restricted`.
...
| 38 | Request paths refuse `.`/`..` key and member segments; `zenodo_read_file` reports one as `member_not_found` before any request, rather than checking members against the listing | Encoding leaves dot segments literal and the URL parser resolves them, so a member path could reach any zenodo.org endpoint with the token attached. A listing check would also refuse real members past the 1,000-node cap, which read fine |
| 39 | Redirects followed manually, only within `https://zenodo.org`, at most 3 hops | Requests reach only zenodo.org whatever Zenodo answers; the runtime's own cross-origin `Authorization` stripping is kept as a second layer, not the guarantee |
| 40 | A malformed query is balanced before it composes with filter clauses, not rejected | Zenodo matches an unparseable `q` as plain text, which drops the filters while the response still claims them. Balancing keeps the filters and the query's plain-text meaning with no new error reason; an interior syntax error remains a known limitation |
| 41 | Parsers and `htmlToText` use forward scans or bounded patterns, never `[…]+$` or `<[^>]*>` over untrusted text | Those forms backtrack quadratically; `creator_orcid` has no length cap before normalization, and descriptions are converted for every search hit |
